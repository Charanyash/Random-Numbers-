...
To solve the above integral, we will use integration by parts, i.e,\\
  ...
  \end{align}
  \begin{align}
	  E\sbrak{x^{2}} &= \frac{1}{\sqrt{2\pi}}\int_{-\infty}^{\infty}x\brak{x\exp\brak{-\frac{x^2}{2}}}dx
			 &= \frac{1}{\sqrt{2\pi}}\brak{x\int x\exp\brak{-\frac{x^2}{2}}dx -  \int\frac{d\brak{x}}{x}\int\exp\brak{-\frac{x^2}{2}}dx}
  \end{align}
For the integral $\int x\exp\brak{-\frac{x^2}{2}}dx$ let us take,
  \begin{align}
	   t &= \frac{x^2}{2}\\
	  dt &= xdx\\
	  \int\exp\brak{-\frac{x^2}{2}}dx &= \int\exp\brak{-t}dt
					  &=  -\exp\brak{-t} + c
				\implies  &= -\exp\brak{-\brak{\frac{x^{2}}{2}}} + c \label{eq:2-6}
  \end{align}
 Using \eqref{eq:2-6},we can write
  \begin{align}
	  E\sbrak{x^{2}} &= \frac{1}{\sqrt{2\pi}}\brak{ -x\exp\brak{-\frac{x^{2}}{2}} + \int\exp\brak{\frac{-x^{2}}{2}}dx}
  \end{align}

  And we know that,
  \begin{align}
	  \frac{1}{\sqrt{2\pi}}\int_{-\infty}^{\infty}\exp\brak{\frac{-x^{2}}{2}} &= 1 \label{eq:2-8}
  \end{align}
